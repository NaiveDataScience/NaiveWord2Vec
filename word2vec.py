...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    grad = y_hat.reshape(y_hat.shape[0],1) * predicted.reshape(1,predicted.shape[0])
    gradPred = y_hat.reshape((1,y_hat.shape[0])).dot(outputVectors).flatten()
    ##raise NotImplementedError
    ### END YOUR CODE
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    predictedVector = inputVectors[currentWordId,:]
    ...
    for word in contextWords:
        index = tokens[word]
        oneCost, oneGradIn, oneGradOut = word2vecCostAndGradient(predictedVector,
                                                                 index, outputVectors, dataset)

        cost += oneCost
        gradIn[currentWordId, :] += oneGradIn
        gradOut += oneGradOut
